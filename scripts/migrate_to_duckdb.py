...
def migrate_history(parquet_path: Path, config_path: Path):
    ...
    for url in tqdm(history_df["url"].unique()):
        if url not in url_to_resource:
            ...
            resource_id = db.add_resource(
                ...
                type="url",
                url=url,
                metadata={"auto_created": True, "from_history": True},
            )
        ...
        for _, row in url_history.iterrows():
            ...
            status_id = db.add_resource_status(
                ...
                url_data={
                    ...
                    "content_hash": (
                        hash_content(str(row.get("content", "")))
                        if row.get("content")
                        else None
                    ),
                },
            ...
            if collection_id:
                ...
                db.add_collection_status(
                    ...
                    status_summary={"urls": {"ok": available, "error": error}},
                )
    ...
    collection_resource_count = db.conn.execute(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--history", default="data/history.parquet", help="Path to history.parquet file"
    )
    parser.add_argument(
        "--config",
