...
def fetch_workflow_runs(owner: str, repo: str, workflow_id: str, token: str) -> List[Dict]:
...
def parse_check_results(log_content: str) -> List[Dict]:
    ...
    skip_urls = {
        'https://da-advisors.github.io/gov-multi-monitor/',
        'artifacts'
    }
    ...
    for line in log_content.split('\n'):
        ...
        # Debug
        if any(p in line.lower() for p in ['url', 'status', 'code', 'time', 'modified', 'error']) or '│' in line:
        ...
        if table_match := re.search(table_row_pattern, line):
            ...
            if url.startswith('http') and not any(skip_url in url for skip_url in skip_urls):
                if current_result:
                ...
                print(f"Found URL and status from table: {current_result['url']} -> {current_result['status']}")
            continue
        ...
        # Parse check details
            ...
            if status_match := re.search(status_pattern, line, re.IGNORECASE):
                ...
                try:
                    current_result['last_modified'] = datetime.fromisoformat(modified_match.group(1).strip())
...
def migrate_logs_to_db(owner: str, repo: str, workflow_id: str, token: str, db: MonitorDB):
        ...
        for result in results:
            ...
            if resource_query:
                ...
                resource_id = db.add_resource(
                    name=result.get('name', url),  # Use name if available, otherwise URL
                    type='url',
                    url=url,
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Migrate GitHub Actions logs to DuckDB")
    parser.add_argument("--owner", required=True, help="Repository owner")
    parser.add_argument("--repo", required=True, help="Repository name")
    parser.add_argument("--workflow", required=True, help="Workflow ID or filename")
    parser.add_argument("--token", default=os.environ.get('GITHUB_TOKEN'), help="GitHub token with actions:read permission. Defaults to GITHUB_TOKEN environment variable.")
    parser.add_argument("--db", default="data/monitor.db", help="Path to DuckDB database")
    
    args = parser.parse_args()
    
    if not args.token:
        parser.error("No GitHub token provided. Either set GITHUB_TOKEN environment variable or use --token")
