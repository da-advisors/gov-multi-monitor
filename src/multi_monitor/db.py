...
class MonitorDB:
    ...
    def __init__(
        self, db_path: Union[str, Path] = "data/monitor.db", read_only: bool = False
    ):
    ...
    def add_resources_to_collection(
        self, collection_id: str, resource_ids: List[str]
    ) -> List[str]:
        ...
        for resource in resource_ids:
            relation_id = self.add_resource_to_collection(
    ...
    def _add_api_result(
        self, resource_id: str, timestamp: datetime, api_result: APICheckResult
    ):
        ...
        if api_result.missing_fields:
            for field in api_result.missing_fields:
                ...
                self.conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, 'missing', ?)
                """,
                    [field_id, resource_id, timestamp, field, api_result.error_message],
                )
    ...
    def get_collection_by_name(self, name: str) -> Optional[Dict]:
        ...
        """
        result = self.conn.execute("SELECT * FROM collections WHERE name = ?", [name])
        row = result.fetchone()
    ...
    def _read_query(self, query_string: str) -> List[tuple]:
        ...
        if self.read_only == False:
            raise (
                """Arbitrary queries are only allowed when the database connection
