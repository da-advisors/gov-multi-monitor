...
from .url_checker import CheckResult
...
class CheckHistory:
    ...
    def add_result(self, result: CheckResult):
        ...
        resources = self.db.conn.execute(
            "SELECT id FROM resources WHERE url = ?", (result.url,)
        ).fetchall()
        ...
        if result.linked_url_results:
            for linked in result.linked_url_results:
                ...
                linked_resources = self.db.conn.execute(
                    "SELECT id FROM resources WHERE url = ?", (linked.url,)
                ).fetchall()
                ...
                linked_url_data = {
                    ...
                    "last_modified": (
                        linked.last_modified.isoformat()
                ...
                print(
                    f"Added linked status {linked_status_id} for resource {linked_resource_id}"
                )
