...
class URLConfig:
    ...
    omb_control_number: Optional[str] = None  # OMB Control Number if applicable
    expected_content: Optional[str] = None
    tags: List[str] = None
    expected_update_frequency: Optional[str] = (
        None  # e.g. "daily", "weekly", "monthly", "quarterly", "yearly"
    )
    ...
    def __post_init__(self):
        ...
        if self.archived_content and isinstance(self.archived_content[0], str):
            self.archived_content = [
                ArchivedContent(url=url) for url in self.archived_content
            ]
        ...
        if "urls" in data:
            ...
            for url_data in data["urls"]:
                ...
                if "archived_content" in url_data:
                    url_data["archived_content"] = [
                        ArchivedContent(**archived_content_data)
                        for archived_content_data in url_data["archived_content"]
                    ]
            ...
            if url_configs_dir.exists():
                ...
                for config_file in sorted(url_configs_dir.glob("*.yaml")):
                    ...
                    if (
                        active_configs is not None
                        and config_file.stem not in active_configs
                    ):
                        continue
                    ...
                    try:
                        with open(config_file) as f:
                            ...
                            if "archived_content" in url_data:
                                url_data["archived_content"] = [
                                    ArchivedContent(**archived_content_data)
                                    for archived_content_data in url_data[
                                        "archived_content"
                                    ]
                                ]
                            urls.append(URLConfig(**url_data))
